...
func (f *crdclient) Create(obj *crd.NetworkObject) (*crd.NetworkObject, error) {
	var result crd.NetworkObject
	err := f.cl.Post().
...
func (f *crdclient) Update(obj *crd.NetworkObject) (*crd.NetworkObject, error) {
	var result crd.NetworkObject
	err := f.cl.Put().
		Namespace(f.ns).Resource(f.plural).
		Body(obj).Do().Into(&result)
...
func (f *crdclient) Get(name string) (*crd.NetworkObject, error) {
	var result crd.NetworkObject
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
...
func (f *crdclient) List(opts meta_v1.ListOptions) (*crd.NetworkObjectList, error) {
	var result crd.NetworkObjectList
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
